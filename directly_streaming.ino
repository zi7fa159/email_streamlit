...
String recordAndTranscribe() {
  ...
  // Safely handle I2S driver uninstallation
  if (i2s_installed) {
    Serial.println("Uninstalling previous I2S driver...");
    i2s_driver_uninstall(I2S_PORT);
    i2s_installed = false;
    // Add a small delay to ensure complete uninstallation
    delay(500);
  }
...
void loop() {
  // For repeated recordings, add a button or timer trigger here
  // For example, to record every 30 seconds:
  // delay(30000);
  // String result = recordAndTranscribe();
  // if (result.length() > 0) {
  //   Serial.println("Final Transcript: " + result);
