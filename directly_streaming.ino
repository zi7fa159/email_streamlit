...
// This function records audio, updates the WAV header, sends it to Deepgram, and returns the transcript.
String recordAndTranscribe() {
  // ----- I2S Initialization (assumes SPIFFS is already mounted) -----
  // Stop any running I2S and then install the driver
  i2s_stop(I2S_PORT);  // Stop I2S if running
  i2s_driver_uninstall(I2S_PORT); // Uninstall previous driver instance if any (ignore warning if not installed)
  ...
  // Write a temporary WAV header (to be updated later)
  ...
  wavHeader[32] = BYTES_PER_SAMPLE; wavHeader[33] = 0;
  wavHeader[34] = SAMPLE_BITS;    wavHeader[35] = 0;
  ...
  client->setInsecure();  // For demo only; implement proper certificate verification in production.
...
void setup() {
  ...
  // Initialize SPIFFS once (do not reinitialize in subsequent calls)
  if (!SPIFFS.begin(true)) {
    Serial.println("ERROR - SPIFFS initialization failed!");
    while (1) delay(1000);
  }
  
  // Call the function only once to avoid repeated reinitializations
...
void loop() {
  // Do nothing here so the function is not called repeatedly.
  delay(1000);
}
